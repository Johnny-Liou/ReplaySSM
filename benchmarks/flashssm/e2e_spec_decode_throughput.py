...
"""End-to-end speculative-decode throughput: AR vs standard spec vs FlashSSM spec.
...
the FlashSSM-spec speedup over both baselines.

  ar       : no speculative decoding
  standard : vLLM native spec decoding (one recurrent state per draft token)
  cache    : FlashSSM cached spec decoding (use_flashssm_spec)
...
MODE_LABEL = {"ar": "AR", "standard": "standard-spec", "cache": "FlashSSM-spec"}


def parse_args():
    p = argparse.ArgumentParser(
        description="E2E spec-decode throughput: AR vs standard vs FlashSSM."
    )
    ...
    p.add_argument("--buffer-len", type=int, default=16,
                   help="FlashSSM buffer length (power of two, >= 1 + num_spec).")
...
def run_worker(args):
    ...
    if mode == "cache":
        llm_kwargs["use_flashssm_spec"] = True
        llm_kwargs["flashssm_buffer_len"] = args.buffer_len
