...
multi-step decode, both ReplaySSM routes reproduce the exact SSM recurrence.
...
def _run_standard_decode(
    *,
    ...
    if desync_write_pos:
        ...
        write_pos = (torch.arange(batch, device=device, dtype=torch.int32)
                     % max_cache_len)
    else:
    ...
    for _ in range(num_steps):
        ...
        out_anchor = selective_state_update_ref(
            state_anchor, x, dt, A, B, C, D=D, z=z, dt_bias=dt_bias,
            dt_softplus=dt_softplus)

        # Upstream baseline decode kernel.
        out_baseline = torch.empty_like(x)
        selective_state_update(
            state_baseline, x, dt, A, B, C, D=D, z=z, dt_bias=dt_bias,
            dt_softplus=dt_softplus, out=out_baseline)
        ...
        common = dict(
            ...
            x_cache=x_cache, dt_cache=dt_cache, B_cache=B_cache,
            write_pos=write_pos, is_flush=is_flush,
            max_cache_len=max_cache_len, out=out_cached)
        if route == "output_only":
            selective_state_update_replayssm_output_only(
                state_cached, x, dt, A, B, C, bc_pre=bc_pre, **common)
            out_ref = selective_state_update_replayssm_output_only_ref(
                state_ref, x, dt, A, B, C, D=D, z=z, dt_bias=dt_bias,
                dt_softplus=dt_softplus, x_cache=x_cache_ref,
                dt_cache=dt_cache_ref, B_cache=B_cache_ref,
                write_pos=write_pos, max_cache_len=max_cache_len)
        else:
            selective_state_update_replayssm_state_and_output(
                state_cached, x, dt, A, B, C, **common)
        ...
        if bool(is_flush.any()):
            torch.testing.assert_close(
                state_cached[is_flush], state_ref[is_flush],
                rtol=rtol, atol=atol)
            if state_dtype == torch.float32:
                torch.testing.assert_close(
                    state_cached[is_flush], state_baseline[is_flush],
                    rtol=rtol, atol=atol)
...
def test_replayssm_standard_decode_with_batch_indices(
    ...
    state = torch.randn(
        total_state_slots, nheads, headdim, dstate, dtype=state_dtype,
        device=device)
    ...
    D = torch.randn(nheads, headdim, device=device)
    x_cache = torch.zeros(
        total_state_slots, nheads, max_cache_len, headdim, device=device,
        dtype=act_dtype)
    dt_cache = torch.zeros(
        total_state_slots, nheads, max_cache_len, device=device,
        dtype=torch.float32)
    B_cache = torch.zeros(
        total_state_slots, ngroups, max_cache_len, dstate, device=device,
        dtype=act_dtype)
    bc_pre = torch.empty(
    ...
    for _ in range(num_steps):
        x = torch.randn(padded_batch, nheads, headdim, device=device,
                        dtype=act_dtype)
        dt = _tied_dt(padded_batch, nheads, headdim, device, act_dtype)
        B = torch.randn(padded_batch, ngroups, dstate, device=device,
                        dtype=act_dtype)
        C = torch.randn(padded_batch, ngroups, dstate, device=device,
                        dtype=act_dtype)
        ...
        selective_state_update(
            state_baseline, x, dt, A, B, C, D=D, z=z, dt_bias=dt_bias,
            dt_softplus=True, state_batch_indices=state_batch_indices,
            out=out_baseline)

        out_cached = torch.full_like(x, 42)
        common = dict(
            D=D, z=z, dt_bias=dt_bias, dt_softplus=True, x_cache=x_cache,
            dt_cache=dt_cache, B_cache=B_cache, write_pos=write_pos,
            is_flush=is_flush, max_cache_len=max_cache_len,
            state_batch_indices=state_batch_indices, out=out_cached)
        if route == "output_only":
            selective_state_update_replayssm_output_only(
                state_cached, x, dt, A, B, C, bc_pre=bc_pre, **common)
        else:
        ...
        if bool(is_flush[:batch].all()):
            torch.testing.assert_close(
                state_cached[state_indices], state_baseline[state_indices],
                rtol=rtol, atol=atol)
