...
@triton.jit
def _flashssm_output_only_precompute_kernel(
    B_ptr,
    C_ptr,
    B_cache_ptr,
    write_pos_ptr,
...
@triton.jit
def _flashssm_output_only_kernel(
...
def _get_flashssm_output_only_launch_config(dstate: int) -> tuple[int, int]:
    if dstate <= 64:
        return 32, 4
    if dstate <= 128:
        return 16, 1
...
def selective_state_update_flashssm_output_only(
    state: torch.Tensor,
    x: torch.Tensor,
    dt: torch.Tensor,
    A: torch.Tensor,
    ...
    block_size_m, num_warps = _get_flashssm_output_only_launch_config(dstate)
    ...
    with torch.accelerator.device_index(x.device.index):
        _flashssm_output_only_precompute_kernel[(batch, ngroups)](
            B,
            C,
            B_cache,
            write_pos,
        ...
        _flashssm_output_only_kernel[grid](
            state,
            x,
            dt,
            dt_bias,
