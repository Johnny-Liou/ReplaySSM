...
# Adapted from FlashMamba's cached-dot selective_state_update_cached.py.
...
@triton.jit
def _selective_scan_update_cached_kernel(
...
def _get_cached_launch_config(dstate: int) -> tuple[int, int]:
    if dstate <= 64:
        return 32, 4
    if dstate <= 128:
        return 32, 1
...
def selective_state_update_cached(
    state: torch.Tensor,
    x: torch.Tensor,
    dt: torch.Tensor,
    A: torch.Tensor,
    ...
    block_size_m, num_warps = _get_cached_launch_config(dstate)
    ...
    with torch.accelerator.device_index(x.device.index):
        _selective_scan_update_cached_kernel[grid](
            state,
            x,
            dt,
            dt_bias,
