...
class MambaStateDtypeCalculator:
    ...
    def mamba2_cached_state_dtype(
        ...
        ``use_flashssm`` is ``False``; otherwise appends the
    ...
    def gated_delta_net_cached_state_dtype(
        ...
        Returns the baseline ``(conv, ssm)`` dtypes when
        ``use_flashssm`` is ``False``; otherwise appends the ring
...
class MambaStateShapeCalculator:
    ...
    def mamba2_cached_state_shape(
        ...
        x_cache_shape = (local_nheads, flashssm_buffer_len, head_dim)
        dt_cache_shape = (local_nheads, flashssm_buffer_len)
        B_cache_shape = (local_ngroups, flashssm_buffer_len, state_size)
        return (
            conv_state_shape,
    ...
    def mamba2_spec_cached_state_shape(
        ...
        ``cache_buf_len = next_pow2(flashssm_buffer_len)`` and ``conv_dim_local``
        ...
        cache_buf_len = 1 << (flashssm_buffer_len - 1).bit_length()
        local_nheads = divide(num_heads, tp_world_size)
    ...
    def gated_delta_net_cached_state_shape(
        ...
        Returns the baseline ``(conv, ssm)`` shapes when
        ``use_flashssm`` is ``False``; otherwise appends the cached
        ...
        d_cache_shape = (local_v_heads, flashssm_buffer_len, head_v_dim)
        k_cache_shape = (local_k_heads, flashssm_buffer_len, head_k_dim)
        g_cache_shape = (local_v_heads, flashssm_buffer_len)
        return (
            conv_state_shape,
            temporal_state_shape,
            d_cache_shape,
